...
function ShiftBadge({ label, type }: { label: string; type: 'week' | 'sat' | 'other' }) {
  ...
  return (
    <span style={{ ...styles[type], display: 'inline-block', padding: '4px 10px', borderRadius: 6, fontSize: 12, fontWeight: 500 }}>
      {label}
    </span>
  )
}
...
function Badge({ label, colour }: { label: string; colour: 'blue' | 'amber' | 'gray' }) {
  ...
  return (
    <span style={{ ...colours[colour], display: 'inline-block', padding: '2px 8px', borderRadius: 6, fontSize: 11, fontWeight: 500 }}>
      {label}
    </span>
  )
}
    ...
    <div onClick={() => setOpen(o => !o)} style={{ background: '#fff', border: open ? '1px solid #185FA5' : '1px solid #e5e5e5', borderRadius: 12, padding: '16px 20px', marginBottom: 10, cursor: 'pointer' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ background: '#E6F1FB', color: '#185FA5', fontWeight: 600, fontSize: 13, padding: '4px 12px', borderRadius: 6 }}
cat > "app/caller/[name]/CallerView.tsx" << 'ENDOFFILE'
'use client'
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9f9f8', fontFamily: "'DM Sans', -apple-system, sans-serif", padding: '32px 16px 64px' }}>
      <div style={{ maxWidth: 600, margin: '0 auto' }}>
        {isStatic && <div style={{ background: '#FAEEDA', color: '#633806', borderRadius: 8, padding: '10px 14px', fontSize: 13, marginBottom: 20 }}>Using static fallback data — Sheets API not connected yet.</div>}
