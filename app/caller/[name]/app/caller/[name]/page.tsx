...
interface CallerData {
  caller: {
    name: string
    shift_week: string | null
    shift_sat: string | null
    shift_other: string | null
  }
...
function ClientCard({ client }: { client: CallerClient }) {
  ...
  return (
    <div
      ...
      style={{
        background: '#fff',
        border: open ? '1px solid #185FA5' : '1px solid #e5e5e5',
        borderRadius: 12,
        padding: '16px 20px',
        marginBottom: 10,
        cursor: 'pointer',
      ...
      {open && (
        ...
        >
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
            {[
              { label: 'Campaign', value: client.campaign || 'TBC' },
              { label: 'SJP client', value: client.sjp || 'N/A' },
            ].map(({ label, value }) => (
              <div key={label} style={{ background: '#f8f8f8', borderRadius: 8, padding: '10px 12px' }}>
                <div style={{ fontSize: 11, color: '#999', textTransform: 'uppercase' as const, letterSpacing: '0.05em', marginBottom: 3 }}>
                  {label}
                </div>
          ...
          {client.script && (
            <>
              <div style={{ fontSize: 11, color: '#999', textTransform: 'uppercase' as const, letterSpacing: '0.05em', marginBottom: 8 }}>
                Opening script
              </div>
              <div style={{
                background: '#f8f8f8',
              ...
              <div style={{ marginBottom: 16 }}>
                {client.calendly.map((a, i) => (
                  <div key={i} style={{
                    ...
                    padding: '9px 0',
                    borderBottom: i < client.calendly.length - 1 ? '1px solid #f0f0f0' : 'none',
                  }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {client.ghl_url && (
              <a href={client.ghl_url} target="_blank" rel="noopener noreferrer" style={{
                display: 'inline-flex', alignItems: 'center', gap: 6,
                padding: '8px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600,
                background: '#185FA5', color: '#E6F1FB', textDecoration: 'none',
              }}>
                Open in GHL ↗
              </a>
            )}
            {client.brief_url && (
              <a href={client.brief_url} target="_blank" rel="noopener noreferrer" style={{
                display: 'inline-flex', alignItems: 'center', gap: 6,
                padding: '8px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600,
...
// ─── Main component (receives data as props from server page) ─────────────────
      ...
      <div style={{ maxWidth: 600, margin: '0 auto' }}>
        ...
        {isStatic && (
          <div style={{
            ...
            Using static fallback data — Sheets API not connected yet.
          </div>
        )}
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{
            ...
            color: '#185FA5', textTransform: 'uppercase', marginBottom: 6,
          }}>
            ADsorbed
          </div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: '#111', marginBottom: 4 }}>
            Good morning, {caller.name}
          </h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 28 }}>
          {[
            { label: 'Clients today', value: clients.length },
